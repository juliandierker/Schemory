...
interface TypeNewPageProps {
  teams: Team[];
}

const DEFAULT_TYPESCRIPT = `export interface ExampleType {
  id: string;
  name: string;
  description?: string;
  createdAt: Date;
...
const TypeNewPage: React.FC<TypeNewPageProps> = ({ teams }) => {
  const { teamId } = useParams<{ teamId: string }>();
  const navigate = useNavigate();
  const [fileName, setFileName] = useState('');
  const [content, setContent] = useState(DEFAULT_TYPESCRIPT);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!fileName.endsWith('.d.ts') && !fileName.endsWith('.ts')) {
      setError('File name should end with .ts or .d.ts');
      return;
    }
    ...
    try {
      const response = await api.createType(teamId!, {
        name: fileName,
        fileName,
        content,
      });
      ...
      if (response.type) {
        toast.success(`Type definition "${fileName}" created!`);
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-secondary-500 dark:text-secondary-400 mt-1">
          Define your TypeScript type structure
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              type="button"
              onClick={() => setContent(DEFAULT_TYPESCRIPT)}
